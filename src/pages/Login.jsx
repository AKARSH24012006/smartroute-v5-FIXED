...
export default function Login({ onAuth }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (data.ok) { localStorage.setItem("sr_token", data.token); onAuth(data.user); return; }
      throw new Error(data.error || "Login failed");
    } catch (err) {
      if (err.message && err.message !== "Login failed" && !err.message.includes("fetch")) {
        setError(err.message);
      } else {
        // Network error / backend not running — use client-side mock
        const name = form.email.split("@")[0];
        onAuth({ name, initials: name.slice(0, 2).toUpperCase(), email: form.email });
      }
    } finally {
      setLoading(false);
    }
