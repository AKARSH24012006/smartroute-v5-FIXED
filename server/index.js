...
import {
  ...
  generateRealisticHotels, generateFlightBookingUrl, generateHotelBookingUrl, generateAllBookingUrls
} from "./lib/api.js";
import { buildMockPlan } from "./lib/planner.js";
import {
  fetchNearbyHotelsOSM, fetchNearbyEmergencyServices, fetchNearbyTransit,
  fetchAlternateAttractions, fetchDestinationWiki, buildBookingLinks,
  detectWeatherAlerts, generateSmartReplan,
} from "./lib/realapis.js";
import {
...
app.post("/api/flights/search", rateLimit(60000, 20), async (req, res) => {
  ...
  const allBookings = generateAllBookingUrls(origin, destination, departure_date, destination);
  const result      = { flights, bookingUrl, bookingPlatforms: allBookings.flights, searchedAt: new Date().toISOString() };
  cache.set(cacheKey, result, 300);
  res.json({ ok:true, ...result });
});

/* ──────────────────────────────────────────────
   HOTELS — with real OYO/Booking/Treebo links
   ────────────────────────────────────────────── */
app.post("/api/hotels/search", rateLimit(60000, 20), async (req, res) => {
  ...
  const allBookings = generateAllBookingUrls("", "", check_in, city);
  const hotelsWithLinks = hotels.map(h => ({
    ...h,
    bookingPlatforms: {
      oyo:        "https://www.oyorooms.com/search?location=" + encodeURIComponent(city),
      makemytrip: "https://www.makemytrip.com/hotels/" + encodeURIComponent(city) + ".html",
      booking:    "https://www.booking.com/search.html?ss=" + encodeURIComponent(h.name),
      treebo:     "https://www.treebohotels.com/hotels-in-" + city.toLowerCase().replace(/\s+/g,"-") + "/",
    },
  }));
  const result = { hotels: hotelsWithLinks, bookingUrl, bookingPlatforms: allBookings.hotels };
  cache.set(cacheKey, result, 300);
  res.json({ ok:true, ...result });
});
...
/* ──────────────────────────────────────────────
   EMERGENCY OPTIONS — REAL DATA via Overpass/OSM
   ────────────────────────────────────────────── */
app.post("/api/emergency-options", async (req, res) => {
  ...
  // Geocode destination to get real coordinates
  let lat = latitude, lon = longitude;
  if ((!lat || !lon) && destination) {
    const geo = await geocodePlace(destination).catch(() => null);
    if (geo) { lat = geo.latitude; lon = geo.longitude; }
  ...
  const cacheKey = `emergency:${dest}:${Math.round(lat||0)}:${Math.round(lon||0)}`;
  const cached = cache.get(cacheKey);
  if (cached) return res.json({ ok: true, ...cached, fromCache: true, dataSource: "OpenStreetMap (cached)" });

  // Fetch real data in parallel from OSM/Overpass
  const [hotelsOSM, emergencyServices, transitStops, bookingLinks] = await Promise.allSettled([
    lat && lon ? fetchNearbyHotelsOSM(lat, lon, 4000) : Promise.resolve([]),
    lat && lon ? fetchNearbyEmergencyServices(lat, lon, 6000) : Promise.resolve([]),
    lat && lon ? fetchNearbyTransit(lat, lon, 2500) : Promise.resolve([]),
    buildBookingLinks(dest, orig, new Date().toISOString().slice(0, 10)),
  ]).then(results => results.map(r => r.status === "fulfilled" ? r.value : []));

  const osmHotels = hotelsOSM.length ? hotelsOSM : [];
  const links = bookingLinks || buildBookingLinks(dest, orig);

  const result = {
    options: {
      // Real OSM hotel data with live booking links
      nearbyHotels: osmHotels.length ? osmHotels.map(h => ({
        name: h.name,
        distKm: h.distKm,
        type: h.type,
        stars: h.stars,
        phone: h.phone,
        website: h.website,
        mapsUrl: `https://www.google.com/maps?q=${h.lat},${h.lon}`,
        bookingUrl: h.bookingUrl,
      })) : [
        { name: `${dest} OYO Rooms`, type: "hotel", bookingUrl: links.hotels.oyo, distKm: "~1.0" },
        { name: `${dest} Treebo`, type: "hotel", bookingUrl: links.hotels.treebo, distKm: "~1.5" },
        { name: `${dest} Booking.com`, type: "hotel", bookingUrl: links.hotels.booking, distKm: "varies" },
      ],

      // Real booking platform deep-links (no API key needed)
      flightLinks: {
        makemytrip: links.flights.makemytrip,
        goibibo:    links.flights.goibibo,
        cleartrip:  links.flights.cleartrip,
        skyscanner: links.flights.skyscanner,
        google:     links.flights.google,
      },
      trainLinks: {
        irctc:     links.trains.irctc,
        paytm:     links.trains.paytm,
        railyatri: links.trains.railyatri,
      },
      cabLinks: {
        ola:    links.cabs.ola,
        uber:   links.cabs.uber,
        rapido: links.cabs.rapido,
      },
      busLinks: {
        redbus:  links.buses.redbus,
        abhibus: links.buses.abhibus,
      },

      // Real emergency services from Overpass/OSM
      emergencyServices: emergencyServices.length ? emergencyServices : [
        { name: "Government Hospital", type: "hospital", phone: "108", distKm: "~2.0" },
        { name: "Police Station", type: "police", phone: "100", distKm: "~1.5" },
      ],

      // Real transit stops from OSM
      transitStops: transitStops.length ? transitStops.slice(0, 5) : [],

      // Static but accurate India emergency contacts
      emergencyContacts: {
        police:           "100",
        ambulance:        "108",
        fire:             "101",
      ...
      insuranceTips: [
        "File a delay certificate at the airline counter immediately",
        "Photograph all receipts — keep originals for insurance claim",
        "Call your card issuer to increase emergency limit if needed",
        "Screenshot hotel/flight confirmations before any cancellations",
        "WhatsApp your hotel your new ETA to hold the reservation",
      ],
    },
    dataSource: lat && lon ? "Live OpenStreetMap + Overpass API" : "Deep-link booking platforms",
    geocoded: lat && lon ? { lat, lon, destination: dest } : null,
...
/* ──────────────────────────────────────────────
   LIVE WEATHER ALERTS — real Open-Meteo data
   ────────────────────────────────────────────── */
app.post("/api/live-alerts", async (req, res) => {
  const { destination, latitude, longitude, days } = req.body || {};
  let lat = latitude, lon = longitude;
  if ((!lat || !lon) && destination) {
    const geo = await geocodePlace(destination).catch(() => null);
    if (geo) { lat = geo.latitude; lon = geo.longitude; }
  }
  if (!lat || !lon) return res.status(400).json({ ok:false, error:"Cannot geocode destination." });

  const cacheKey = `alerts:${Number(lat).toFixed(2)}:${Number(lon).toFixed(2)}:${days||5}`;
  const cached = cache.get(cacheKey);
  if (cached) return res.json({ ok:true, ...cached, fromCache:true });

  const weatherData = await fetchWeather(lat, lon, Number(days)||7).catch(() => []);
  const enriched = weatherData.map(d => ({ ...d, emoji: weatherEmoji(d.weatherCode) }));
  const alerts = detectWeatherAlerts(enriched);

  const result = {
    alerts,
    weather: enriched,
    hasAlerts: alerts.length > 0,
    shouldReplan: alerts.some(a => a.shouldReplan),
    destination,
    geocoded: { lat, lon },
    dataSource: "Open-Meteo (free, no key)",
    fetchedAt: new Date().toISOString(),
  };
...
/* ──────────────────────────────────────────────
   SMART REPLAN — weather-triggered replanning
   ────────────────────────────────────────────── */
app.post("/api/replan", async (req, res) => {
  const { destination, origin, itinerary, latitude, longitude, days } = req.body || {};
  if (!destination) return res.status(400).json({ ok:false, error:"Destination required." });
  ...
  if (!lat || !lon) return res.status(400).json({ ok:false, error:"Cannot geocode destination for replanning." });

  const [weatherData, alternateVenues] = await Promise.allSettled([
    fetchWeather(lat, lon, Number(days)||7),
    fetchAlternateAttractions(lat, lon, 5000),
  ]).then(r => r.map(x => x.status === "fulfilled" ? x.value : []));

  const enrichedWeather = weatherData.map(d => ({ ...d, emoji: weatherEmoji(d.weatherCode) }));
  const alerts = detectWeatherAlerts(enrichedWeather);
  const replan = generateSmartReplan(itinerary || [], alerts, alternateVenues, destination);
  const links = buildBookingLinks(destination, origin, new Date().toISOString().slice(0, 10));
  ...
  res.json({
    ...
    alternateVenues: alternateVenues.slice(0, 8),
    bookingLinks: links,
    dataSource: "Open-Meteo + Overpass/OpenStreetMap (free, no key)",
    destination,
    fetchedAt: new Date().toISOString(),
  });
});
...
/* ──────────────────────────────────────────────
   DESTINATION INFO — Wikipedia + OSM
   ────────────────────────────────────────────── */
app.get("/api/destination-info", async (req, res) => {
  ...
  const cacheKey = `destinfo:${destination.toLowerCase()}`;
  const cached = cache.get(cacheKey);
  if (cached) return res.json({ ok:true, ...cached, fromCache:true });

  const [wikiInfo, geo] = await Promise.allSettled([
    fetchDestinationWiki(destination),
    geocodePlace(destination),
  ]).then(r => r.map(x => x.status === "fulfilled" ? x.value : null));

  let pois = [];
  if (geo?.latitude && geo?.longitude) {
    pois = await fetchAlternateAttractions(geo.latitude, geo.longitude, 6000).catch(() => []);
  }
  ...
  const result = {
    ...
    wiki: wikiInfo,
    geocode: geo,
    topAttractions: pois.slice(0, 10),
    dataSource: "Wikipedia REST + Overpass/OpenStreetMap",
    fetchedAt: new Date().toISOString(),
  };
