...
const KNOWN_COORDS = {
  ...
  // SRM / Kattankulathur / South Chennai area
  kattankulathur: { name: "Kattankulathur",  country: "India", latitude: 12.8231, longitude: 80.0444 },
  srm:            { name: "SRM University", country: "India", latitude: 12.8231, longitude: 80.0444 },
  potheri:        { name: "Potheri",        country: "India", latitude: 12.8240, longitude: 80.0430 },
  varkala:        { name: "Varkala",        country: "India", latitude: 8.7379,  longitude: 76.7163 },
  hampi:          { name: "Hampi",          country: "India", latitude: 15.335,  longitude: 76.462  },
};
...
export function generateFlightBookingUrl(origin, destination, date) {
  ...
  // Primary: MakeMyTrip with proper flight search params
  return `https://www.makemytrip.com/flights/search?itinerary=${enc(origin||"DEL")}-${enc(destination||"BOM")}-${dt}&tripType=O&paxType=A-1_C-0_I-0&intl=false&cabinClass=E`;
}

export function generateHotelBookingUrl(city, checkIn, checkOut) {
  const enc = encodeURIComponent;
  const citySlug = (city||"").toLowerCase().replace(/\s+/g, "-");
  // Primary: OYO Rooms deep-link for Indian cities
  return `https://www.oyorooms.com/search?location=${enc(city||"")}`;
}

export function generateAllBookingUrls(origin, destination, date, city) {
  const enc = encodeURIComponent;
  const dt = date || new Date().toISOString().slice(0, 10);
  const dtFmt = dt.replace(/-/g, "");
  const citySlug = (city||destination||"").toLowerCase().replace(/\s+/g, "-");
  return {
    flights: {
      makemytrip: `https://www.makemytrip.com/flights/search?itinerary=${enc(origin||"DEL")}-${enc(destination||"BOM")}-${dt}&tripType=O&paxType=A-1_C-0_I-0&intl=false&cabinClass=E`,
      goibibo:    `https://www.goibibo.com/flights/search/${enc(origin||"DEL")}-${enc(destination||"BOM")}-${dtFmt}-1-0-0-E-f-0-0-0-0-0/`,
      cleartrip:  `https://www.cleartrip.com/flights/results/?from=${enc(origin||"DEL")}&to=${enc(destination||"BOM")}&depart_date=${dt}&adults=1&class=Economy`,
      google:     `https://www.google.com/travel/flights?q=flights+from+${enc(origin||"")}+to+${enc(destination||"")}`,
    },
    hotels: {
      oyo:        `https://www.oyorooms.com/search?location=${enc(city||destination||"")}`,
      makemytrip: `https://www.makemytrip.com/hotels/${enc(city||destination||"")}.html`,
      booking:    `https://www.booking.com/search.html?ss=${enc(city||destination||"")}&dest_type=city`,
      treebo:     `https://www.treebohotels.com/hotels-in-${citySlug}/`,
    },
    trains: {
      irctc:      `https://www.irctc.co.in/nget/train-search?fromStation=${enc(origin||"")}&toStation=${enc(destination||"")}`,
      railyatri:  `https://www.railyatri.in/train-tickets/${enc(origin||"").toLowerCase()}-to-${enc(destination||"").toLowerCase()}`,
    },
    cabs: {
      ola:        `https://book.olacabs.com/?pickup=${enc(origin||"")}&drop=${enc(destination||"")}`,
      uber:       `https://m.uber.com/looking`,
    },
    buses: {
      redbus:     `https://www.redbus.in/bus-tickets/${(origin||"").toLowerCase().replace(/\s+/g,"-")}-to-${(destination||"").toLowerCase().replace(/\s+/g,"-")}`,
    },
  };
}
